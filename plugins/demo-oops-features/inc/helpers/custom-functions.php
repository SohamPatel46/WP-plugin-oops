<?php
/**
 * Demo_oops features custom functions.
 *
 ...
 */
function demo_oops_get_cache_key( $unique = '' ) {

	$cache_key = 'demo_oops_cache_';
 ...
 */
function demo_oops_get_cached_posts( $args ) {
	...
	$cache_key = demo_oops_get_cache_key( $args );

	$cache  = new \Demo_oops\Features\Inc\Cache( $cache_key );
 ...
 */
function demo_oops_get_template_content( $slug, $vars = [] ) {
 ...
 */
function demo_oops_features_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', DEMO_OOPS_FEATURES_PATH, $template );
 ...
 */
function demo_oops_get_data( $slug, $default = [] ) {

	$data_file = sprintf( DEMO_OOPS_FEATURES_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function demo_oops_is_production() {
 ...
 */
function demo_oops_is_mobile( $kind = 'any', $return_matched_agent = false ) {
