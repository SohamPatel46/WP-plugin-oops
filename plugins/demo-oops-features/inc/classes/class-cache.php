...
class Cache {
	 ...
	 * @var string
	 */
	const ERROR_CODE = 'demo_oops_cache';
	 ...
	 * @var string
	 */
	protected static $cache_group = 'demo_oops_cache_v1';
	...
	public function __construct( $cache_key = '', $cache_group = '' ) {
		...
		if ( empty( $cache_key ) || ! is_string( $cache_key ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'Cache key is required to create cache object', 'demo-oops-features' ) );
		}
	...
	protected function init() {
		 ...
		 */
		$cache_group = apply_filters( 'demo_oops_cache_group_override', self::$cache_group, $this->key );
	...
	public function updates_with( $callback, $params = [] ) {
		...
		if ( empty( $callback ) || ! is_callable( $callback ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'Callback passed is not callable', 'demo-oops-features' ) );
		}

		if ( ! is_array( $params ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'All parameters for the callback must be in an array', 'demo-oops-features' ) );
		}
	...
	public function get() {
		...
		if ( empty( $this->callback ) || ! is_callable( $this->callback ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'No valid callback set', 'demo-oops-features' ) );
		}
